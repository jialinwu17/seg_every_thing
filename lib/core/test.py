...
def im_detect_all(model, im, box_proposals, save_name, timers=None):
    ...
    scores, boxes, cls_boxes, fc6, roi_feat = box_results_with_nms_and_limit(scores, boxes, fc6, roi_feat)
    ...
    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
        ...
        if cfg.TEST.MASK_AUG.ENABLED:
            ...
            masks, masks_roi_feat, masks_fcn1, masks_fcn2, masks_fcn3, masks_fcn4 = im_detect_mask(model, im_scale, boxes)
        timers['im_detect_mask'].toc()

        timers['misc_mask'].tic()
        cls_segms, feat_masks = segm_results(
    ...
    masks_roi_feat = (masks_roi_feat * feat_masks).sum(2).sum(2) / (feat_masks.sum(2).sum(2) + 0.0000001)
    masks_fcn1 = (masks_fcn1 * feat_masks).sum(2).sum(2) / (feat_masks.sum(2).sum(2) + 0.0000001)
    masks_fcn2 = (masks_fcn2 * feat_masks).sum(2).sum(2) / (feat_masks.sum(2).sum(2) + 0.0000001)
    masks_fcn3 = (masks_fcn3 * feat_masks).sum(2).sum(2) / (feat_masks.sum(2).sum(2) + 0.0000001)
    masks_fcn4 = (masks_fcn4 * feat_masks).sum(2).sum(2) / (feat_masks.sum(2).sum(2) + 0.0000001)
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    fc6 = workspace.FetchBlob(core.ScopedName('fc6'))
    roi_feat = workspace.FetchBlob(core.ScopedName('roi_feat')).mean(2).mean(2)
    # 1000, 1024 1000, 256
    # In case there is 1 proposal
    #scores = scores.reshape([-1, scores.shape[-1]])
    ...
    return scores, pred_boxes, im_scale, fc6, roi_feat
...
def box_results_with_nms_and_limit(scores, boxes, fc6, roi_feat):
    ...
    cls_fc6 = [[] for _ in range(num_classes)]
    cls_roi_feat = [[] for _ in range(num_classes)]
    ...
    for j in range(1, num_classes):
        ...
        if cfg.TEST.SOFT_NMS.ENABLED:
            ...
            nms_fc6_j = fc6_j[keep, :]
            nms_roi_feat_j = roi_feat_j[keep, :]
    ...
    if cfg.TEST.DETECTIONS_PER_IM > 0:
        ...
        if len(image_scores) > cfg.TEST.DETECTIONS_PER_IM:
            ...
            for j in range(1, num_classes):
                ...
                cls_fc6[j] = cls_fc6[j][keep, :]
                cls_roi_feat[j] = cls_roi_feat[j][keep, :]

    im_results = np.vstack([cls_boxes[j] for j in range(1, num_classes)])
    im_fc6 = np.vstack([cls_fc6[j] for j in range(1, num_classes)])
    im_roi_feat = np.vstack([cls_roi_feat[j] for j in range(1, num_classes)])
    boxes = im_results[:, :-1]
    scores = im_results[:, -1]
    return scores, boxes, cls_boxes, im_fc6, im_roi_feat
...
def segm_results(cls_boxes, masks, ref_boxes, im_h, im_w):
    ...
    feat_masks = np.zeros((masks.shape[0], 1 , 14, 14), dtype = np.uint8)
    ...
    for j in range(1, num_classes):
        ...
        for _ in range(cls_boxes[j].shape[0]):
            ...
            w = np.maximum(w, 1)
            h = np.maximum(h, 1)
            tmp = cv2.resize(masks[mask_ind, j, :, :], (14, 14))
            feat_masks[mask_ind, 0, :, :] = np.array(tmp > cfg.MRCNN.THRESH_BINARIZE, dtype=np.uint8)
